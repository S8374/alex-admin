...
export const ApplicationDetail = ({ 
  ...
  const { user, pets, personInfo, representative, healthAnswers, status, createdAt, familyHealthHistories } = application;
          ...
          <div className="flex flex-row items-center gap-2 sm:gap-3 w-full lg:w-auto mt-4 lg:mt-0">
            {status !== "APPROVED" && status !== "REJECTED" && (
              <>
                ...
                >
                  {isUpdating ? <div className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 border-2 border-rose-600 border-t-transparent rounded-full animate-spin" /> : <XCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2" />}
                  Reject
                </Button>
                <Button 
                ...
                >
                  {isUpdating ? <div className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2" />}
                  Approve
                </Button>
              </>
            )}
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-2 flex-wrap justify-end">
                  <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 font-bold text-xs px-3">
                    {processedHealthAnswers.length} Total
                  </Badge>
