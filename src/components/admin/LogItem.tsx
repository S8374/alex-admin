                  ...
                  return (
                    value && (
                      <div key={key} className={`flex flex-col gap-1 bg-gray-50 px-3 py-2 rounded-xl border border-gray-100 max-w-full ${isJson ? 'w-full' : ''}`}>
                        ...
                        <pre className={`text-[10px] font-bold text-gray-700 break-words whitespace-pre-wrap leading-relaxed ${isJson ? 'font-mono bg-white p-3 rounded-lg border border-gray-100' : ''} ${shouldTruncate ? 'max-h-24 overflow-hidden' : ''}`}>
                          {jsonString}
                        </pre>
                        {shouldTruncate && (
                          <div className="h-4 bg-gradient-to-t from-white to-transparent -mt-4 relative z-10" />
                        )}
                      </div>
                    )
                  );
        ...
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-10">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-5xl max-h-[80vh] bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-gray-100"
            >
                ...
                {Object.entries(log.details).map(([key, value]: [string, any]) => {
                   ...
                   return (
                     <div key={key} className="space-y-4">
                       ...
                       <pre className="text-xs font-bold text-gray-700 bg-white p-6 rounded-2xl border border-gray-100 font-mono overflow-x-auto shadow-sm">
